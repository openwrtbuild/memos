...
func (d *DB) UpdateAttachment(ctx context.Context, update *store.UpdateAttachment) error {
	...
	defer func() { _ = tx.Rollback() }()
	actor, err := requireMySQLActiveMemoActor(ctx, tx, update.Policy.ActorUserID)
	if err != nil {
		return err
	}
	...
	if err := authorizeMySQLAttachmentMutation(ctx, tx, update.Policy.ActorUserID, actor.Admin, memoIDs, nil); err != nil {
		return err
	}
