...
`)
			...
			cmd.Env = append(os.Environ(), "PATH="+mockBin+string(os.PathListSeparator)+os.Getenv("PATH"), "TEST_FIXTURE="+dir,
				"TEST_TAG="+tag, "TEST_DOWNLOAD_LOG="+logPath, "MEMOS_VERSION=", "MEMOS_SKIP_CHECKSUM=0", "REPO=openwrtbuild/memos")
			out, err = cmd.CombinedOutput()
			require.NoError(t, err, string(out))
			log, err := os.ReadFile(logPath)
			require.NoError(t, err)
			require.Contains(t, string(log), "https://github.com/openwrtbuild/memos/releases/download/"+tag+"/"+asset)
